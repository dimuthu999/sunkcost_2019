...
sample_trans_main <- readRDS(file="C:/Users/dnratnadiwakara/Documents/sunkcost_2019/ztraxdata/raw/trans_main_sample.rds")
trans_main <- trans_main[,names(sample_trans_main),with=FALSE]
gc()
...
sample_trans_property <- readRDS(file="C:/Users/dnratnadiwakara/Documents/sunkcost_2019/ztraxdata/raw/trans_property_sample.rds")
trans_property <- trans_property[,names(sample_trans_property),with=FALSE]
gc()
...
asmt_main <- merge(asmt_main,trans_main, by="ImportParcelID")



asmt_main[,Purchase_month:=as.Date(paste(substr(asmt_main$DocumentDate,1,7),"-01",sep=""))]
asmt_main[,Purchase_year:=as.numeric(format(asmt_main$DocumentDate,"%Y"))]

merged_data<-asmt_main[,c("ImportParcelID","RowID","StreetAddress","City","zip",
...
merged_data <- merge(merged_data,zhvi,by=c("zip","Purchase_month"))
  ...
  names(zhvi) <- c("zip","HPI_asmt","Current_month")
merged_data[,Current_month:=as.Date("2016-01-01")]
merged_data <- merge(merged_data,zhvi,by=c("zip","Current_month"))
...
merged_data[,tax_2014:=ifelse(merged_data$TaxYear==2014,merged_data$TaxAmount,NA)]
merged_data[,tax_2015:=ifelse(merged_data$TaxYear==2015,merged_data$TaxAmount,NA)]
merged_data[,tax_2016:=ifelse(merged_data$TaxYear==2016,merged_data$TaxAmount,NA)]

saveRDS(merged_data,file=paste("C:/Users/dnratnadiwakara/Documents/sunkcost_2019/all_homes_prev/",state_code,"_all_homes_prev.rds",sep=""))
