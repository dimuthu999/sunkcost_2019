...
ggplot()+geom_line(data=year_summary[!year_summary$State %in% c("AZ")],aes(x=Purchase_year,y=median_tax*100,linetype=factor(State)))+theme_minimal()+theme(legend.position="none")+
...
regs <- list()
regs[[1]] <- felm(as.formula(paste("tax_rate~Ownership_years+log(Adj_purch_price)+log(nominal_loss)+Bedrooms+Bathrooms+log(House_age)|zip|0|zip")),data=all_homes_prev[all_homes_prev$State=="CA" & all_homes_prev$Purchase_year<2008 & all_homes_prev$House_age>0])
regs[[2]] <- felm(as.formula(paste("tax_rate~Ownership_years+log(Adj_purch_price)+log(nominal_loss)+Bedrooms+Bathrooms+log(House_age)|zip|0|zip")),data=all_homes_prev[all_homes_prev$State=="IL" & all_homes_prev$Purchase_year<2008])
regs[[3]] <- felm(as.formula(paste("tax_rate~Ownership_years+log(Adj_purch_price)+log(nominal_loss)+log(House_age)|zip|0|zip")),data=all_homes_prev[all_homes_prev$State=="NY" & all_homes_prev$Purchase_year<2008 ])
...
regs <- list()
regs[[1]] <- felm(as.formula(paste("prop_tax_rate~ownership_years+",controls,fe,"0",cluster,sep="")),data=zdata_reg[zdata_reg$PurchaseYear<2008 ])
regs[[2]] <- felm(as.formula(paste("prop_tax_rate~ownership_years+",controls,"+medianage+medianhouseholdincome+Renter_fraction",fe,"0",cluster,sep="")),data=zdata_reg[zdata_reg$PurchaseYear<2008 ])
...
printtable(regs,column.labels = c("1996-2007", "1996-2007",">2011","All"),note="",
           lines=list(c("FE:","Zip x Month","Zip x Month","Zip x Sale Month","Zip x Sale Month"),c("Cluster:","Zip","Zip","Zip","Zip")))
...
```{r}
regs <- list()
regs[[1]] <- felm(as.formula(paste("log(SalesPriceAmount_sale)~Ownership_years+","log(Adj_purch_price)+log(nominal_loss)+Bedrooms+Bathrooms+log(House_age)+BuildingAreaSqFt","|zip_sale_month|","0","|zip",sep="")),data=CA_all[CA_all$Purchase_year<2008 & CA_all$Sale_year>2014 & CA_all$House_age>2 & CA_all$Sale_premium<1.3 ])
...
printtable(regs,column.labels = c("CA","IL","NY"),note="",
           lines=list(c("FE:","Zip x Month","Tract x Month","Zip x Month","Zip x Sale Month"),c("Cluster:","Zip","Zip","Zip","Zip")))
```
...
adj_proptax <- felm(Property_tax_rate_last_year~0|zip_purch_list,data=temp)
...
rdplot(temp$adj_proptax_resid,temp$purch_week,p=poly,c=br,x.label = "Purchased week (June 01 = 0)",title = "Property tax rate",y.label = "")
rdplot(temp$adj_list_resid,temp$purch_week,p=poly,c=br,x.label = "Purchased week (June 01 = 0)",title = "log(List price)*",y.label = "")
rdplot(temp$adj_sale_resid,temp$purch_week,p=poly,c=br,x.label = "Purchased week (June 01 = 0)",title = "log(Sale price)*",y.label = "")
...
regs <- list()
regs[[1]] <- felm(as.formula(paste("Property_tax_rate_last_year~purchase_after_june01+log(adj_purch_price)",controls,sep="")),data=temp)
